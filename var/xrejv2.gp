...
set xlabel "test run (each with $2^{30}$ extended precision random matrices)"

set ylabel "maximal relative errors in $\\varepsilon$"
...
plot "xjv2t.csv" using 1:4 with points title "$\\mathop{\\mathrm{\\rho_{\\mathbb{R}}}}(\\det V)$" pointtype 13 linetype 14, \
     ...
     "wjv2t.csv" using 1:7 with points title "$\\mathop{\\mathrm{\\rho_{\\mathbb{C}}}}(\\Im\\sinh\\phi)$" pointtype 5 linetype 18, \
     "wjv2t.csv" using 1:6 with points title "$\\mathop{\\mathrm{\\rho_{\\mathbb{C}}}}(\\Re\\sinh\\phi)$" pointtype 7 linetype -1
