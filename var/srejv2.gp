...
set xlabel "test run (each with $2^{30}$ single precision random matrices)"
...
plot "sjv2t.csv" using 1:4 with points title "$\\mathop{\\mathrm{\\rho_{\\mathbb{R}}}}(\\det V)$" pointtype 13 linetype 14, \
     ...
     "cjv2t.csv" using 1:7 with points title "$\\mathop{\\mathrm{\\rho_{\\mathbb{C}}}}(\\Im\\sinh\\phi)$" pointtype 5 linetype 18, \
     "cjv2t.csv" using 1:6 with points title "$\\mathop{\\mathrm{\\rho_{\\mathbb{C}}}}(\\Re\\sinh\\phi)$" pointtype 7 linetype -1
