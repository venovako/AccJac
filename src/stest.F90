PROGRAM STEST
...
#endif
     INFO = 0
     CALL SJIEV2(A, B, C, RT1, RT2, CS1, SN1, INFO)
     IF (INFO .NE. 0) THEN
#ifndef NDEBUG
        WRITE (*,'(A,I2)') 'SJIEV2=', INFO
#endif
        CYCLE
     END IF
#ifndef NDEBUG
  ...
  END DO
  CLOSE(U)
  WRITE (*,9) 'SJAEV2:min((det(U)-1)/ε)=', MJD
  WRITE (*,9) 'SJAEV2:max((det(U)-1)/ε)=', XJD
  WRITE (*,9) 'SLAEV2:min((det(U)-1)/ε)=', MLD
  WRITE (*,9) 'SLAEV2:max((det(U)-1)/ε)=', XLD
