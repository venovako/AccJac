...
SUBROUTINE MTRANA(N, A, LDA, V, LDV, P, Q, TOL, INFO)
  ...
  CALL MPFR_INIT_F(C)
  CALL MPFR_INIT_F(S)
  CALL MPFR_SET_F(A1, A(P,P))
  CALL MPFR_SET_F(A2, A(Q,Q))
  ...
  CALL MPFR_SET_ZERO(TOL, 0_c_int)
  IF (S .LT. C) THEN
     INFO = 0
  ELSE ! rotate
     IF (I .EQ. 0) THEN
     ...
     ELSE ! transf
        I = 2
     END IF
     CALL MPFR_SET_F(A(P,P), A1)
     CALL MPFR_SET_F(A(Q,Q), A2)
     IF ((I .EQ. 2) .AND. (IAND(INFO, 2) .EQ. 0)) CALL MPFR_SET_ZERO(A(Q,P), 0_c_int)
     INFO = I
  END IF
  CALL MPFR_CLEAR_F(S)
  CALL MPFR_CLEAR_F(C)
  CALL MPFR_CLEAR_F(A2)
  CALL MPFR_CLEAR_F(A1)
