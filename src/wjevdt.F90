...
#include "cr.f90"
  ...
  EXTERNAL :: BFOPEN, WBRD1, WBWR1, WJEVDC, WJEVDR, YCLEAR
  ...
  DO J = 1, N
#ifdef __GFORTRAN__
     WRITE (I,'(ES30.21E4)') REAL(A(J,J))
#else
     WRITE (I,'(ES45.36E4)') REAL(A(J,J))
#endif
  END DO
  CLOSE (UNIT=I, IOSTAT=J)
  ...
  END IF
#ifdef __GFORTRAN__
  WRITE (OUTPUT_UNIT,'(ES30.21E4)') REAL(W, K)
#else
  WRITE (OUTPUT_UNIT,'(ES45.36E4)') REAL(W, K)
#endif
  DEALLOCATE(Z)
  DEALLOCATE(Y)
  DEALLOCATE(X)
