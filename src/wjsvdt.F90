PROGRAM WJSVDT
  ...
  COMPLEX(KIND=c_long_double) :: T
  INTEGER(KIND=INT64) :: CLK(3)
  INTEGER :: M, N, LDG, LDV, JPOS, GS, INFO, I, J, L
  COMPLEX(KIND=c_long_double), ALLOCATABLE :: G(:,:), V(:,:), WRK(:,:)
  REAL(KIND=c_long_double), ALLOCATABLE :: SV(:)
  COMPLEX(KIND=REAL128), ALLOCATABLE :: U(:,:), W(:,:)
  EXTERNAL :: BFOPEN, WJSVDC, WJSVDR
  ...
  IF (IAND(L, 4) .EQ. 0) THEN
     CALL WJSVDC(M, N, G, LDG, V, LDV, JPOS, SV, WRK, GS, INFO)
  ELSE ! WJSVDR
     CALL WJSVDR(M, N, G, LDG, V, LDV, JPOS, SV, WRK, GS, INFO)
  END IF
...
#else
  STOP 'wjsvdt.exe must be compiled with GNU Fortran on x64'
#endif
END PROGRAM WJSVDT
