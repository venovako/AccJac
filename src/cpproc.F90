PROGRAM CPPROC
  ...
  DO L = 1, S
     ...
     IF (N .EQ. 1) THEN
        WRITE (U,'(ES16.9E2)') PR(1,1)
     ELSE ! N > 1
        DO I = 1, N
           DO J = 1, N-1
              IF (J .EQ. 1) THEN
                 WRITE (U,'(ES16.9E2)',ADVANCE='NO') PR(I,1)
              ELSE ! J > 1
                 WRITE (U,'(ES17.9E2)',ADVANCE='NO') PR(I,J)
              END IF
           END DO
           WRITE (U,'(ES17.9E2)') PR(I,N)
        END DO
     END IF
     CLOSE(UNIT=U, IOSTAT=V)
     ...
     DO J = 2, N
        ...
        END DO
     END DO
     WRITE (OUTPUT_UNIT,'(ES16.9E2,A)',ADVANCE='NO') X, ','
     FLUSH(OUTPUT_UNIT)
     X = XZERO
     DO J = 2, N
        DO I = 1, J-1
