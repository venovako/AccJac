...
#include "cr.f90"
  ...
  ELSE ! parallel
     ...
     ORD = 0
     O = 0
     CALL JSWEEP(L, N, I, J, TBL, O)
     IF (O .NE. 0) STOP 'JSWEEP'
     CALL ZJEVDM(N, A, N, V, N, JPOS, WRK, AS, TBL, ORD, INFO)
     DEALLOCATE(ORD)
     DEALLOCATE(TBL)
  END IF
  ...
  DO J = 1, N
     DO I = 1, N
        Y(I,J) = A(I,J) - X(I,J)
        W = CR_HYPOT(W, CR_HYPOT(REAL(Y(I,J)), AIMAG(Y(I,J))))
     END DO
  END DO
  !$OMP END PARALLEL DO
  IF (W .NE. XZERO) THEN
     ...
     DO J = 1, N
        DO I = 1, N
           M = CR_HYPOT(M, CR_HYPOT(REAL(REAL(A(I,J)), KK), REAL(AIMAG(A(I,J)), KK)))
        END DO
     END DO
     !$OMP END PARALLEL DO
     W = W / M
