PROGRAM SJSVDT
  ...
  IF (IAND(L, 4) .EQ. 0) THEN
     CALL SJSVDC(M, N, G, LDG, V, LDV, JPOS, SV, WRK, GS, INFO)
  ELSE ! SJSVDR
     CALL SJSVDR(M, N, G, LDG, V, LDV, JPOS, SV, WRK, GS, INFO)
  END IF
  Y = (PVN_TIME_MONO_NS() - Y) / 1000000000
  WRITE (OUTPUT_UNIT,'(I11,A,I6,A,F12.6,A)',ADVANCE='NO') INFO, ',', GS, ',', Y, ','
  FLUSH(OUTPUT_UNIT)
  IF (INFO .LT. 0) THEN
     IF (IAND(L, 4) .EQ. 0) THEN
        STOP 'SJSVDC'
