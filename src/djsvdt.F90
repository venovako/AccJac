PROGRAM DJSVDT
  ...
  IF (IAND(L, 4) .EQ. 0) THEN
     CALL DJSVDC(M, N, G, LDG, V, LDV, JPOS, SV, WRK, GS, INFO)
  ELSE ! DJSVDR
     CALL DJSVDR(M, N, G, LDG, V, LDV, JPOS, SV, WRK, GS, INFO)
  END IF
  Y = (PVN_TIME_MONO_NS() - Y) / 1000000000
  T = WRK(1,1)
  WRITE (OUTPUT_UNIT,'(I11,A,I12,A,I6,A,F12.6,A)',ADVANCE='NO') INFO, ',', TRANSFER(T, 0_INT64), ',', GS, ',', Y, ','
  FLUSH(OUTPUT_UNIT)
  IF (INFO .LT. 0) THEN
     IF (IAND(L, 4) .EQ. 0) THEN
        STOP 'DJSVDC'
