PROGRAM ZTEST
  ...
  COMPLEX(c_double), TARGET :: A, B, C, SN1
  REAL(c_double), TARGET :: RT1, RT2, CS1
  ...
  DO WHILE (I .LE. N)
     A = DRSAFE(U)
     B = CMPLX(DRSAFE(U), DRSAFE(U), c_double)
     C = DRSAFE(U)
#ifndef NDEBUG
...
#endif
     INFO = 0
     CALL ZJIEV2(A, B, C, RT1, RT2, CS1, SN1, INFO)
     IF (INFO .NE. 0) THEN
#ifndef NDEBUG
        WRITE (*,'(A,I2)') 'ZJIEV2=', INFO
#endif
        CYCLE
     END IF
#ifndef NDEBUG
  ...
  WRITE (*,9) 'ZLAEV2:min((det(U)-1)/ε)=', MLD
  WRITE (*,9) 'ZLAEV2:max((det(U)-1)/ε)=', XLD
