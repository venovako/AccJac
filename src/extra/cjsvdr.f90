...
SUBROUTINE CJSVDR(M, N, G, LDG, V, LDV, JPOS, SV, WRK, GS, INFO)
  ...
  INTERFACE
     PURE SUBROUTINE CINISV(M, N, G, LDG, V, LDV, JPOS, SV, WRK, INFO)
       ...
       COMPLEX(KIND=REAL32), INTENT(INOUT) :: G(LDG,N)
       COMPLEX(KIND=REAL32), INTENT(OUT) :: V(LDV,N), WRK(M,N)
       REAL(KIND=REAL32), INTENT(OUT) :: SV(N)
       INTEGER, INTENT(INOUT) :: INFO
     END SUBROUTINE CINISV
  END INTERFACE
  INTERFACE
     SUBROUTINE CTRANS(M, N, G, LDG, V, LDV, SV, GX, GS, P, Q, TOL, INFO)
       ...
       COMPLEX(KIND=REAL32), INTENT(INOUT) :: G(LDG,N), V(LDV,N)
  ...
  ! init SV, V; sort SV, G
  R = IAND(INFO, 2)
  CALL CINISV(M, N, G, LDG, V, LDV, JPOS, SV, WRK, R)
  IF (R .LT. 0) THEN
     INFO = -9
     RETURN
  END IF
  ...
  DO R = 1, S
     ...
     ELSE ! asc
        DO Q = N, JPOS+2, -1
           ...
           DO P = Q-1, JPOS+1, -1
              W = IAND(INFO, 1)
              CALL CTRANS(M, N, G, LDG, V, LDV, SV, GX, GS, Q, P, TOL, W)
              SELECT CASE (W)
              CASE (0)
                 CONTINUE
              CASE (1,2,3)
