  ...
  GX = ZERO
  O = -1
  CALL GSCALG(M, N, G, LDG, GX, GS, O)
  IF (O .LT. 0) THEN
     INFO = -3
     GOTO 9
  END IF
  R = INFO
  CALL GINISX(M, N, G, LDG, V, LDV, SV, IX, R)
  IF (R .NE. 0) THEN
     INFO = -10
     GOTO 9
  END IF
  ...
  DO R = 1, S
     ...
     ELSE ! SLOW
        O = 0
     END IF
     CALL GPRCYC(M, N, G, LDG, JPOS, SV, IX, WRK, RWRK, O)
     IF (O .LT. 0) THEN
        INFO = -8
        GOTO 9
     END IF
     ...
     DO ST = 1, TS
        ...
        W = 0
        X = ZERO
        !$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(O,P,Q,Y) SHARED(M,N,G,LDG,V,LDV,JPOS,SV,IX,WRK,RWRK,ORD,INFO,TP,TOL,L,U) REDUCTION(MAX:W,X)
        DO O = 1, TP
           P = ORD(1,O)
           Q = ORD(2,O)
           Y = TOL
