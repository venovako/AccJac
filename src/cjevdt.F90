...
#include "cr.f90"
  ...
  !$OMP DECLARE REDUCTION(HYP:REAL(KK):OMP_OUT=CR_HYPOTD(OMP_OUT,OMP_IN)) INITIALIZER(OMP_PRIV=XZERO)
  ...
  IF (W .NE. XZERO) THEN
     ...
     W = W / M
  END IF
  WRITE (OUTPUT_UNIT,'(ES16.9E2)') REAL(W, K)
  DEALLOCATE(Z)
  DEALLOCATE(Y)
  DEALLOCATE(X)
  DEALLOCATE(WRK)
