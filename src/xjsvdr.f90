...
SUBROUTINE XJSVDR(M, N, G, LDG, V, LDV, JPOS, SV, GS, INFO)
  IMPLICIT NONE
  INTERFACE
  ...
  END INTERFACE
  INTERFACE
     PURE SUBROUTINE XINISV(M, N, G, LDG, V, LDV, JPOS, SV, INFO)
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: M, N, LDG, LDV, JPOS
       REAL(KIND=10), INTENT(INOUT) :: G(LDG,N)
       REAL(KIND=10), INTENT(OUT) :: V(LDV,N), SV(N)
  ...
  INTERFACE
     SUBROUTINE XTRANS(M, N, G, LDG, V, LDV, SV, GX, GS, P, Q, TOL, INFO)
       ...
       REAL(KIND=10), INTENT(INOUT) :: G(LDG,N), V(LDV,N), SV(N), GX
  ...
  ! init SV, V; sort SV, G
  R = IAND(INFO, 2)
  CALL XINISV(M, N, G, LDG, V, LDV, JPOS, SV, R)
  IF (R .LT. 0) THEN
     INFO = -3
     RETURN
  END IF
